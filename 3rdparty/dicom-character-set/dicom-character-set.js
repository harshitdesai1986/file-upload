/*! dicom-character-set - 1.0.1 - 2018-07-17 | (c) 2018 Radialogica, LLC | https://github.com/radialogica/dicom-character-set */
...
exports.convertBytesPromise = convertBytesPromise;

var _characterSets = __webpack_require__(/*! ./character-sets.js */ "./character-sets.js");

var _characterSets2 = _interopRequireDefault(_characterSets);
...
function appendRunWithPromise(output, byteRunCharacterSet, bytes, byteRunStart, byteRunEnd) {
  ...
  return (output === '' ? _promise2.default.resolve('') : output).then(function (lhs) {
...
function convertBytesCore(withoutExtensionsFunc, appendFunc, specificCharacterSet, bytes, options) {
  ...
  if (characterSetStrings.length === 1 && !characterSetStrings[0].startsWith('ISO 2022')) {
    return withoutExtensionsFunc(_characterSets2.default[characterSetStrings[0]].encoding, bytes);
  }

  var checkedOptions = options || {};

  return convertWithExtensions(characterSetStrings.map(function (characterSet) {
    return _characterSets2.default[characterSet];
...
function convertWithExtensions(allowedCharacterSets, bytes, delimiters, appendRun) {
  ...
  var initialCharacterSets = {
    G0: allowedCharacterSets[0].elements.filter(function (element) {
      return element.codeElement === 'G0';
    })[0],
    G1: allowedCharacterSets[0].elements.filter(function (element) {
      return element.codeElement === 'G1';
    })[0]
  };

  var activeCharacterSets = (0, _assign2.default)({}, initialCharacterSets);
  var byteRunStart = 0;
...
function convertWithoutExtensions(encoding, bytes) {
  var retVal = new TextDecoder(encoding).decode(bytes);
...
function filterMultiByteCharacterSetStrings(characterSetStrings) {
  var initialCharacterSet = _characterSets2.default[characterSetStrings[0]];
  ...
  return characterSetStrings.filter(function (str) {
    return !_characterSets2.default[str].multiByte || _characterSets2.default[str].extension;
  });
}
...
function findNextCharacterSet(bytes, start, currentCodeElement, activeCodeElements, initialCharacterSets, delimiters) {
  for (var i = start; i < bytes.length; i += currentCodeElement.bytesPerCodePoint) {
    ...
    if (currentCodeElement.bytesPerCodePoint === 1 && delimiters.indexOf(bytes[i]) >= 0) {
      (0, _assign2.default)(activeCodeElements, initialCharacterSets);
    }
...
function forceExtensionsIfApplicable(characterSetStrings) {
  ...
  try {
    for (var _iterator = (0, _getIterator3.default)(characterSetStrings), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var characterSetString = _step.value;

      if (returnValue.indexOf(characterSetString) < 0) {
...
function getCharacterSet(byte, activeCharacterSets) {
  ...
  return _characterSets2.default['ISO 2022 IR 6'].elements[0];
}
...
function getCharacterSetStrings(specificCharacterSet) {
  ...
  if (characterSetStrings.some(function (characterSet) {
    return _characterSets2.default[characterSet] === undefined;
  })) {
...
function preprocessBytes(characterSet, bytes, byteStart, byteEnd) {
  ...
  if (characterSet.isJISX0212) {
    ...
    oneEncodingBytes = bytes.subarray(byteStart, byteEnd);
...
function readEscapeSequence(bytes, start, extensionSets) {
  ...
  try {
    for (var _iterator2 = (0, _getIterator3.default)(extensionSets), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var extensionSet = _step2.value;
      var _iteratorNormalCompletion3 = true;
      var _didIteratorError3 = false;
      var _iteratorError3 = undefined;

      try {
        for (var _iterator3 = (0, _getIterator3.default)(extensionSet.elements), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
...
var _convertBytes = __webpack_require__(/*! ./convert-bytes.js */ "./convert-bytes.js");

Object.defineProperty(exports, 'convertBytes', {
  enumerable: true,
  get: function get() {
    return _convertBytes.convertBytes;
  }
});
Object.defineProperty(exports, 'convertBytesPromise', {
  enumerable: true,
  get: function get() {
    return _convertBytes.convertBytesPromise;
  }
});
